...
export default function WorkoutsPage() {
  ...
  const handleGenerate = async () => {
    ...
    setGenerating(true);
    try {
      toast.loading("Generating workout + matching diet plan...", { id: "gen" });
      ...
      const workoutPlan: WorkoutPlan = {
        userId: user.uid,
        plan_name: data.workout?.plan_name || `${goals} Workout Plan`,
      ...
      setPlan(workoutPlan);
      setShowForm(false);
      toast.success("Workout plan + matching diet plan created!", { id: "gen" });
    } catch (err: unknown) {
    ...
    <div className="space-y-6">
      ...
      {showForm && (
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 space-y-4">
          ...
          <p className="text-sm text-gray-500">A matching diet plan will be auto-generated based on your workout.</p>
          ...
          <button onClick={handleGenerate} disabled={generating}
            ...
            {generating ? <><Loader2 className="w-5 h-5 animate-spin" /> Generating Plans (may take 1-2 min)...</> : <><Dumbbell className="w-5 h-5" /> Generate Workout + Diet Plan</>}
          </button>
        </div>
      )}
      ...
      {plan && plan.weekly_schedule && plan.weekly_schedule.length > 0 && (
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-sm text-gray-500">
            ...
            {plan.source === "auto_from_diet" && <span className="bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full text-xs">Auto-generated from diet</span>}
          </div>
              ...
              {expandedDay === day.day && !day.is_rest_day && (
                <div className="px-6 pb-4 space-y-3">
                  {day.warmup && day.warmup.length > 0 && (
                    <div>
                      <p className="text-xs font-semibold text-gray-400 uppercase mb-1">Warmup</p>
                      {day.warmup.map((w, i) => (
                        <p key={i} className="text-sm text-gray-600">{w.exercise} — {w.duration_sec}s</p>
                      ))}
                    </div>
                  )}
                  {day.exercises && day.exercises.map((ex, i) => (
                    <div key={i} className="p-3 bg-gray-50 rounded-xl">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full">{ex.muscle_group}</span>
                      </div>
                  ...
                  {day.cooldown && day.cooldown.length > 0 && (
                    <div>
                      <p className="text-xs font-semibold text-gray-400 uppercase mb-1">Cooldown</p>
                      {day.cooldown.map((c, i) => (
                        <p key={i} className="text-sm text-gray-600">{c.stretch} — {c.duration_sec}s</p>
                      ))}
                    </div>
                  )}
          ...
          {plan.trainer_notes && (
            <div className="bg-blue-50 rounded-2xl p-4 border border-blue-100">
              <p className="text-sm font-semibold text-blue-800 mb-1">Trainer Notes</p>
              <p className="text-sm text-blue-700">{plan.trainer_notes}</p>
            </div>
          )}
          {plan.nutrition_timing && (
            <div className="bg-green-50 rounded-2xl p-4 border border-green-100">
              <p className="text-sm font-semibold text-green-800 mb-1">Nutrition Timing</p>
              <p className="text-sm text-green-700">{plan.nutrition_timing}</p>
            </div>
          )}
        </div>
      )}
